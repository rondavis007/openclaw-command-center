...
var require_llm_usage = __commonJS({
  "src/llm-usage.js"(exports2, module2) {
    ...
    function parseUsageTextFallback(stdout) {
      const text = String(stdout || "");
      const out = {
        timestamp: (/* @__PURE__ */ new Date()).toISOString(),
        source: "live-text",
        claude: {
          session: { usedPct: null, remainingPct: null, resetsIn: null, error: null },
          weekly: { usedPct: null, remainingPct: null, resets: null, error: null },
          sonnet: { usedPct: null, remainingPct: null, resets: null, error: null },
      ...
      const claudeErr = text.match(/Claude:\s*(HTTP\s+\d+:[^\n]+)/i);
      if (claudeErr) {
        out.claude.session.error = claudeErr[1].trim();
        out.claude.weekly.error = claudeErr[1].trim();
        out.claude.sonnet.error = claudeErr[1].trim();
      }
      const codex5h = text.match(/Codex[^\n]*\n\s*5h:\s*(\d+)%\s+left\s+·\s+resets\s+([^\n]+)/i);
      if (codex5h) {
        out.codex.usage5hPct = Math.max(0, 100 - Number(codex5h[1]));
        out.codex.reset5h = codex5h[2].trim();
      }
      const codexWeek = text.match(/\n\s*Week:\s*(\d+)%\s+left\s+·\s+resets\s+([^\n]+)/i);
      if (codexWeek) {
        out.codex.usageDayPct = Math.max(0, 100 - Number(codexWeek[1]));
        out.codex.resetDay = codexWeek[2].trim();
      }
      return out;
    }
    function refreshLlmUsageAsync() {
      ...
      const profile = process.env.OPENCLAW_PROFILE || "";
      const args2 = profile ? ["--profile", profile, "status", "--usage"] : ["status", "--usage"];
      execFile(
        "openclaw",
        args2,
        ...
        (err, stdout) => {
          ...
          try {
            ...
            if (jsonStart >= 0) {
              ...
              if (parsed.usage) {
                ...
                console.log("[LLM Usage] Cache refreshed (json)");
                return;
              }
            }
          } catch {
          ...
          try {
            const result = parseUsageTextFallback(stdout);
            llmUsageCache.data = result;
            llmUsageCache.timestamp = Date.now();
            console.log("[LLM Usage] Cache refreshed (text)");
          } catch (e) {
    ...
    function transformLiveUsageData(usage) {
      ...
      const anthropicError = anthropic?.error || null;
      ...
      return {
        timestamp: (/* @__PURE__ */ new Date()).toISOString(),
        source: anthropicError ? "partial" : "live",
        error: anthropicError,
        errorType: anthropicError ? anthropicError.includes("403") || anthropicError.toLowerCase().includes("unauthorized") || anthropicError.toLowerCase().includes("forbidden") ? "auth" : anthropicError.includes("429") || anthropicError.toLowerCase().includes("rate limited") ? "rate_limit" : "unknown" : null,
        claude: {
          session: {
            usedPct: anthropicError ? null : Math.round(session5h?.usedPercent || 0),
            remainingPct: anthropicError ? null : Math.round(100 - (session5h?.usedPercent || 0)),
            resetsIn: anthropicError ? null : formatReset(session5h?.resetAt),
            error: anthropicError
          },
          weekly: {
            usedPct: anthropicError ? null : Math.round(weekAll?.usedPercent || 0),
            remainingPct: anthropicError ? null : Math.round(100 - (weekAll?.usedPercent || 0)),
            resets: anthropicError ? null : formatReset(weekAll?.resetAt),
            error: anthropicError
          },
          sonnet: {
            usedPct: anthropicError ? null : Math.round(sonnetWeek?.usedPercent || 0),
            remainingPct: anthropicError ? null : Math.round(100 - (sonnetWeek?.usedPercent || 0)),
            resets: anthropicError ? null : formatReset(sonnetWeek?.resetAt),
            error: anthropicError
          },
          lastSynced: anthropicError ? null : (/* @__PURE__ */ new Date()).toISOString()
        },
        codex: {
          sessionsToday: 0,
          tasksToday: 0,
