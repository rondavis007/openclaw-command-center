  ...
  "src/sessions.js"(exports2, module2) {
    ...
    function createSessionsModule2(deps) {
      ...
      async function refreshSessionsCache() {
        ...
        try {
          const output = await runOpenClawAsync2("sessions --all-agents --json 2>/dev/null");
      ...
      function getSessions(options = {}) {
        ...
        try {
          const output = runOpenClaw2("sessions --all-agents --json 2>/dev/null");
          const jsonStr = extractJSON2(output);
          if (jsonStr) {
            const data = JSON.parse(jsonStr);
      ...
      function getSessionDetail(sessionKey) {
        try {
          const listOutput = runOpenClaw2("sessions --all-agents --json 2>/dev/null");
          ...
          let channelDisplay = deps.humanSessionLabel?.(sessionKey, sessionInfo?.agentId || "main") || sessionInfo.groupChannel || sessionInfo.displayName || "Session";
...
var require_llm_usage = __commonJS({
  "src/llm-usage.js"(exports2, module2) {
    ...
    function transformLiveUsageData(usage) {
      ...
      if (anthropic?.error) {
        return {
          ...
          errorType: anthropic.error.includes("403") ? "auth" : anthropic.error.includes("429") || anthropic.error.toLowerCase().includes("rate") ? "rate_limit" : "unknown",
          claude: {
    ...
    function getLlmUsage2(statePath) {
      ...
      const anthropicScrapeFile = path2.join(statePath, "anthropic-usage.json");
      const bundledAnthropicScrapeFile = path2.join(__dirname, "..", "public", "data", "anthropic-usage.json");
...
// src/session-labels.js
var require_session_labels = __commonJS({
  "src/session-labels.js"(exports2, module2) {
    var fs2 = require("fs");
    var path2 = require("path");
    var os = require("os");
    var { execFileSync } = require("child_process");
    var HOME = os.homedir();
    var DISCORD_CACHE_TTL_MS = 15 * 60 * 1e3;
    var DISCORD_CACHE_PATH = path2.join(HOME, ".openclaw", "state", "discord-channel-cache.json");
    var _guildIds = ["1471930469141446718"];
    ...
    function readChannelCache() {
      try {
        return JSON.parse(fs2.readFileSync(DISCORD_CACHE_PATH, "utf8"));
      } catch {
    ...
    function writeChannelCache(cache) {
      try {
        fs2.mkdirSync(path2.dirname(DISCORD_CACHE_PATH), { recursive: true });
        fs2.writeFileSync(DISCORD_CACHE_PATH, JSON.stringify(cache, null, 2));
      } catch {
      }
    }
    var _fetchInProgress = false;
    var FAILED_IDS_CACHE_PATH = DISCORD_CACHE_PATH.replace(/\.json$/, ".failed-ids.json");
    function _loadFailedIds() {
      try {
        const raw = JSON.parse(fs2.readFileSync(FAILED_IDS_CACHE_PATH, "utf8"));
    ...
    function _saveFailedIds(set) {
      try {
        fs2.mkdirSync(path2.dirname(FAILED_IDS_CACHE_PATH), { recursive: true });
        fs2.writeFileSync(FAILED_IDS_CACHE_PATH, JSON.stringify([...set], null, 2));
      } catch {
      }
    }
    var _lookupFailedIds = _loadFailedIds();
    var _extraCache = (() => {
      try {
    ...
    async function getDiscordChannelMetadata2(openclawConfig) {
    ...
    async function resolveUnknownChannels2(sessionKeys, openclawConfig, channelMetadata) {
      const ids = [
        ...new Set(
          sessionKeys.map(
            (k) => k.includes(":discord:channel:") ? k.split(":discord:channel:")[1]?.split(":")[0] : null
          ).filter(Boolean)
        )
      ];
      ...
      for (const id of idsToLookup) {
        try {
          const out = execFileSync(
            ...
            { encoding: "utf8", timeout: 1e4, stdio: ["pipe", "pipe", "ignore"] }
          );
          const jsonStart = out.indexOf("{");
          const parsed = JSON.parse(jsonStart >= 0 ? out.slice(jsonStart) : out);
    ...
    function humanSessionLabel(sessionKey = "", agentId = "main", channelMetadata = {}, cronJobsById = {}) {
      ...
      if (sessionKey.includes(":discord:channel:")) {
        ...
        return agentId === "main" ? base : `${titleCase(agentId)} \xB7 ${base}`;
      }
      ...
      if (sessionKey.includes(":subagent:")) return `Sub-agent \xB7 ${titleCase(agentId)}`;
    ...
    module2.exports = {
      getDiscordChannelMetadata: getDiscordChannelMetadata2,
      resolveUnknownChannels: resolveUnknownChannels2,
      humanSessionLabel,
...
var { getDiscordChannelMetadata, resolveUnknownChannels, humanSessionLabel: _humanSessionLabel } = require_session_labels();
...
var _openclawConfig = null;
var _discordChannelMeta = {};
var _cronJobsById = {};
...
setInterval(() => refreshDiscordMeta().catch(() => {
}), 15 * 60 * 1e3);
setTimeout(async () => {
  try {
    const allSessions = sessions.getSessions({ limit: null });
    const keys = (Array.isArray(allSessions) ? allSessions : allSessions?.sessions || []).map((s) => s.sessionKey).filter(Boolean);
...
var CRASH_LOG = "/tmp/command-center-crash.log";
...
`;
  try {
    fs2.appendFileSync(CRASH_LOG, msg, "utf8");
  } catch {
  }
  console.error(msg);
}
...
process.on("exit", (code) => {
  ...
  const ts = (/* @__PURE__ */ new Date()).toISOString();
  try {
    fs2.appendFileSync(CRASH_LOG, `[${ts}] EXIT code=${code}
`, "utf8");
  } catch {
  }
});
